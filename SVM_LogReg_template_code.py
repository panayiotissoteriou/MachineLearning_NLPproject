        ...
        for line in infile:
            ...
            if components[-1] != "_" and components[-1] != "V":  # we only want the arguments classified
                ...
                feature_dict = {'Token': token, 'PoS': pos, 'Lemma': lemma, 'dependency': dependency, 'head': head,
                                'dependent': dependent, 'constituent': constituent, 'Passive': passive,
...
def create_report(gold_labels, predictions):
    ...
    print(
        'Features used: token, pos, lemma, dependency, head, dependent,\n constituent, passive_voice, possible_ARG, head_predicate')
    print()
    print(report)
...
def classification_report_csv(gold_labels, predictions):
    ...
    clsf_report = pd.DataFrame(
        classification_report(y_true=gold_labels, y_pred=predictions, output_dict=True)).transpose()
